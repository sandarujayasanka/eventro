...
<head>
    ...
    <title>Wedding Services | Eventro</title>
    ...
    <style>
        ...
        .hero-bg {
            background: url('../assets/w1.jpg') center/cover no-repeat;
            position: relative;
        }
        .hero-bg::before {
            content: '';
...
<div class="hero-bg text-white py-20">
    <div class="hero-content max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <div class="mb-6">
            <i class="fas fa-heart text-6xl mb-4 opacity-90"></i>
        </div>
        <h1 class="text-5xl md:text-6xl font-bold mb-6">Wedding Services</h1>
        <p class="text-xl mb-8 opacity-90 max-w-3xl mx-auto">Make your special day truly unforgettable with our complete wedding services — from planning to perfection.</p>
        
        <!-- Quick Stats -->
        <div class="flex justify-center items-center space-x-8 text-lg">
            <div class="flex items-center">
                <i class="fas fa-users mr-2"></i>
                <span><?= mysqli_num_rows($result_services) ?>+ Wedding Services</span>
            </div>
            <div class="flex items-center">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div class="flex flex-col lg:flex-row gap-4 items-start lg:items-center">
            <div class="flex items-center space-x-2">
                ...
                <div>
                    <span class="font-semibold text-gray-900 block">Find weddings services in your area</span>
                </div>
            </div>
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-8">
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <h3 class="text-lg font-semibold text-gray-900 mb-4">Popular Weddings Events </h3>
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
            <?php 
            $photo_types = [
                ['name' => 'Poruwa Ceremony', 'icon' => 'fas fa-ring', 'color' => 'blue'],
                ['name' => 'Homecoming', 'icon' => 'fas fa-heart', 'color' => 'purple'],
                ['name' => 'Reception', 'icon' => 'fas fa-glass-cheers', 'color' => 'yellow'],
                ['name' => 'Pre-wedding Shoot', 'icon' => 'fas fa-camera', 'color' => 'green'],
                ['name' => 'Engagement', 'icon' => 'fas fa-gem', 'color' => 'red'],
                ['name' => 'Bridal Dressing', 'icon' => 'fas fa-crown', 'color' => 'pink'],
                ['name' => 'Grooms Dressing', 'icon' => 'fas fa-user-tie', 'color' => 'blue'],
                ['name' => 'Ashtaka', 'icon' => 'fas fa-book-open', 'color' => 'green'],
                ['name' => 'Wes Dancing', 'icon' => 'fas fa-drum', 'color' => 'red'],
                ['name' => 'Jayamangala Gatha', 'icon' => 'fas fa-praying-hands', 'color' => 'green']
            ];
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
    <?php if (mysqli_num_rows($result_services) > 0): ?>
        <div class="mb-6">
            <h2 class="text-2xl font-bold text-gray-900">
                Weddings Services <?= !empty($selected_district) ? 'in ' . htmlspecialchars($selected_district) : '' ?>
            </h2>
            <p class="text-gray-600">Found <?= mysqli_num_rows($result_services) ?> wedding service<?= mysqli_num_rows($result_services) !== 1 ? 's' : '' ?></p>
        </div>
    ...
    <?php else: ?>
        <div class="text-center py-16">
            ...
            <h3 class="text-xl font-semibold text-gray-700 mb-2">No Weddings Events found</h3>
            <p class="text-gray-500">
                <?php if ($selected_district): ?>
                    No Weddings Events found in <?= htmlspecialchars($selected_district) ?>. Try selecting a different location.
                <?php else: ?>
                    No Weddings Events are currently available. Check back later!
                <?php endif; ?>
            </p>
        </div>
    <?php endif; ?>
