...
<head>
    ...
    <style>
        ...
        .hero-bg {
            background: url('../assets/b1.jpg') center/cover no-repeat;
            position: relative;
        }
        .hero-bg::before {
            content: '';
...
<div class="hero-bg text-white py-20">
    <div class="hero-content max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <h1 class="text-5xl md:text-6xl font-bold mb-6">Birthday Services</h1>
        ...
        <div class="flex justify-center items-center space-x-8 text-lg">
            <div class="flex items-center">
                ...
                <span><?= mysqli_num_rows($result_services) ?>+ Birthday Services</span>
            </div>
            <div class="flex items-center">
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-8">
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <h3 class="text-lg font-semibold text-gray-900 mb-4">Popular Birthday Events </h3>
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
            <?php 
            $photo_types = [
                ['name' => 'Kids Birthday', 'icon' => 'fas fa-child', 'color' => 'blue'],
                ['name' => 'Sweet 16', 'icon' => 'fas fa-star', 'color' => 'purple'],
                ['name' => '18th Birthday', 'icon' => 'fas fa-key', 'color' => 'yellow'],
                ['name' => '21st Birthday', 'icon' => 'fas fa-wine-glass', 'color' => 'green'],
                ['name' => 'Milestone Birthdays', 'icon' => 'fas fa-medal', 'color' => 'red']
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
    <?php if (mysqli_num_rows($result_services) > 0): ?>
        <div class="mb-6">
            <h2 class="text-2xl font-bold text-gray-900">
                Birthday Services <?= !empty($selected_district) ? 'in ' . htmlspecialchars($selected_district) : '' ?>
            </h2>
            <p class="text-gray-600">Found <?= mysqli_num_rows($result_services) ?> birthday<?= mysqli_num_rows($result_services) !== 1 ? 's' : '' ?></p>
        </div>
    ...
    <?php else: ?>
        <div class="text-center py-16">
            <i class="fas fa-birthday-cake text-gray-300 text-6xl mb-4"></i>
            <h3 class="text-xl font-semibold text-gray-700 mb-2">No Birthday Events found</h3>
            <p class="text-gray-500">
                <?php if ($selected_district): ?>
                    No Birthday Events found in <?= htmlspecialchars($selected_district) ?>. Try selecting a different location.
                <?php else: ?>
                    No Birthday Events are currently available. Check back later!
                <?php endif; ?>
            </p>
        </div>
    <?php endif; ?>
